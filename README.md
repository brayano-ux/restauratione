...
<body>
    ...
    <script type="module">
        ...
        async function creerCompte() {
            ...
            if (!nom || !numero || !mdp) return Swal.fire('Attention', 'Remplis tous les champs', 'warning');
            if (mdp.length < 6) return Swal.fire('Attention', 'Mot de passe trop court (min 6 caractères)', 'warning');
        ...
        async function envoyer() {
            ...
            if (!titre || !prix || !description) return Swal.fire('Attention', 'Remplis tous les champs', 'warning');
            const user = auth.currentUser;
            if (!user) return Swal.fire('Attention', 'Connecte-toi', 'warning');

            const coords = await new Promise((res, rej) => navigator.geolocation.getCurrentPosition(res, rej));
        ...
        async function envoyerMessage() {
            ...
            // Correction : crée le doc parent pour la conversation privée
            if (!idJobEnCours) {
                ...
                if (convId.includes(currentUser.uid)) {
                    ...
                    div.innerHTML = `
                        ...
                        <button class="btn btn-primary" onclick="window.ouvrirChatPriveGeneral('${currentUser.uid}', '${otherUid}')">
                            <i clmass="fas fa-comments"></i> Ouvrir chat
                        </button>
                    `;
                    container.appendChild(div);
                }
